...
サイズ固定が出来るようになって(Workers AI でも出来るものはある)、出力が安定した気がするが、ほしい感じの画像はまあでない。
...
この手の画像の生成をやったことがある人は知っていると思うのですが、生成 AI が作るドット絵はドット絵ではありません。わからないと何を言っているんだという感じですが、この記事のサムネの元になった画像を見てみます。
...
差額で見た感じ、ここにすべてを載せているわけではないが、今回の機能を作るのに $25 くらいかかっていた。
今回土日の可処分時間でやったもので、全部自分で書いたらこうはならなかったなと思うと時間を買えた感覚はあるのだが、今後の趣味の開発がこのペースで進むとして単純に 4 週で月 $100 + α。
...
これはまあわかっていた話というか、別に普段の開発でも変わらないところ。今回でいうとドット絵の編集のところ。
...
これは index.html にすべての実装を乗せていったのが完全に悪いのだけど、最後の方は 3 回くらい往復したらコンテキストが足りなくなる、みたいになっていた。
後で実際のやりとりを除いてみると、ほぼ毎回 index.html をめちゃくちゃ送っている、でかい。ファイル指定しているからそりゃそう。

今回これを回避するのに、前のプロンプト貼り付けて、多分続きまでやってるからなんとかして、みたいなことをしていた。
この進め方自体はあれだが、 Roo Code はそれでも結構察して進めてくれるので、素直にすごいなと思った。
